...
library(car)
durbinWatsonTest(mfit) #D-W Statistic 2.317691 : 2에 가까울 수록 독립성 만족
...
#--------------------------------------------------------------------------
# 예측(독립, 설명)변수 선택 : 분석가의 입장에서 가장 고민해야할 문제중 하나
mfit1 <-lm(Murder ~ .,, data = states)
summary(mfit1) # Income, Frost 변수는 의미없는 변수일 가능성이 높다.
# Adjusted R-squared:  0.5285 <- 결정계수 값

mfit2 <-lm(Murder ~ Population+Illiteracy, data = states)
summary(mfit2)
# Adjusted R-squared:  0.5484 <- 결정계수 값
...
summary(fwd_model)
#위의 방법은 일반적이나, 기회를 갖지 못하는 변수가 있을수 있다는 단점이있다.
